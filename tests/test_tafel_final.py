...
def test_tafel_with_dataframe():
    ...
    # 4. Check results using actual key names from your engine
    ...
    # Basic assertions (using actual key names)
    ...
    assert 0.05 < result['beta_a'] < 0.25, f"Beta_a out of range: {result['beta_a']}"
    assert 0.05 < result['beta_c'] < 0.25, f"Beta_c out of range: {result['beta_c']}"
...
def test_tafel_manual_data():
    ...
    assert icorr_error < 10, f"Icorr error too high: {icorr_error:.1f}%"
    assert beta_error < 10, f"Beta error too high: {beta_error:.1f}%"
    assert result['cr'] > 0
